...
export async function GET() {
  try {
    const pool = getPool();
    const [major, allMajor, middle, eventTypes] = await Promise.all([
      pool.query("select code, code_name from sw002_com_code where grp_cd = 'CATE_001' and attr1 = 'Y' order by code"),
      pool.query("select code, code_name from sw002_com_code where grp_cd = 'CATE_001' order by code"),
    ...
    return Response.json({ majorCategories: major.rows, allMajorCategories: allMajor.rows, middleCategories: middle.rows, eventTypes: eventTypes.rows });
  } catch (error) {
