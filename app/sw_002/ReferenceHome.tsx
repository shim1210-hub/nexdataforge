...
type ReferenceStore = {
  ...
  events: Array<{ title: string; description: string | null; endAt: string | null; mapIcon: string | null }>;
};
...
export default function ReferenceHome() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/sw_002/api/store-categories", { cache: "no-store" }).then((response) => response.json() as Promise<{ majorCategories?: CategoryOption[] }>),
    ])
