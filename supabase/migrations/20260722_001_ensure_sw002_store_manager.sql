-- Ensure the documented store-manager test account can authenticate.
-- The password is "1", stored using the same scrypt format as the app.
...
begin
  update sw002_users
     set password_hash = 'eb416f30e399b4fd1c532eda80ffb16f:eb07502a7589135763994198f7b71f835b3b6ab05ca49aebf85bdad342e30745549a016426e4542e62c69d47d7956ebf6dba4caf9bd3f07aff9c53ed9395b9dd',
         nickname = coalesce(nullif(nickname, ''), 'Store Manager'),
         role = 'STORE_MANAGER',
         status = 'ACTIVE',
         updated_at = now()
   where lower(email) = 'pa@naver.com';
